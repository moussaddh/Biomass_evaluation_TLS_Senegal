...
df = let
    csv_files =
        filter(
            ...
            readdir(joinpath("../0-data", "1.2-mtg_manual_measurement_corrected_enriched/acacia"), join=true)
        )
...
formula_all = @formula(cross_section ~ 0 + cross_section_pipe + axis_length)
...
# ╔═╡ e9c79ea7-0fd5-4060-b9cb-78e47c510b22
md"Show values for plantscan3d? $(@bind show_ps3d PlutoUI.CheckBox(default=true))"
...
CSV.write("../2-results/1-data/0-wood_density_acacia.csv", df_density)
...
begin
    dir_path_lidar = joinpath("..", "0-data", "3-mtg_lidar_plantscan3d", "2-manually_corrected", "acacia", "0-branch")
    dir_path_lidar_raw = nothing
    dir_path_manual = joinpath("..", "0-data", "1.2-mtg_manual_measurement_corrected_enriched", "acacia")
    dir_path_lidar_new = joinpath("..", "0-data", "3-mtg_lidar_plantscan3d", "3-corrected_enriched", "acacia")
    mtg_files =
        filter(
            x -> splitext(basename(x))[2] in [".mtg"],
            readdir(dir_path_lidar)
        )
	filter!(x -> x != "ar_Rs6_cor.mtg", mtg_files) # Not measured by hand
end
...
# ╔═╡ 3a16dbde-e04e-4a0e-8e23-ff357a8729e9
md"""
!!! note
	We have only two points here because the other branches with the full MTG were not scanned with the lidar. They are only used for building the model.
"""
...
df_training = let
    ...
    full_mtg_files = ["ar_R10.csv", "ar_R13.csv", "ar_Rs14.csv", "ar_Rs18.csv"]
    csv_files = [joinpath("../0-data", "1.2-mtg_manual_measurement_corrected_enriched/acacia", i) for i in full_mtg_files]
...
CSV.write("../2-results/1-data/4-structural_model_acacia.csv", coeftable(model))
...
save("../2-results/2-plots/cross_section_evaluation_acacia.png", article_figure_CSA; px_per_unit=3)
...
CSV.write("../2-results/1-data/2-stats_cross_section_acacia.csv", stats_cross_section)
...
function structural_model!(mtg_tree, fresh_density, dry_density, first_cross_section=nothing; model=structural_model_acacia)
...
begin
	...
	!show_ps3d && filter!(row -> !endswith(row.variable, "ps3d"), df_mass)
end;
...
let
	...
	!show_ps3d && filter!(row -> !endswith(row.variable, "ps3d"), df_)
    plt_volume_branches =
        abline +
        data(df_) *
        mapping(
...
let
    plt_mass_branches =
        abline +
        #data(filter(row -> row.origin == "Validation" && row.variable != "fresh_mass_ps3d", df_mass)) *
...
stats_evaluation = let
    stats_mass =
        combine(
            ...
            [:fresh_mass_manual, :value] => nRMSE => :nRMSE,
            [:fresh_mass_manual, :value] => EF => :EF,
            [:fresh_mass_manual, :value] => Bias => :Bias,
            [:fresh_mass_manual, :value] => RME => :RME
        )
...
manifest_format = "2.0"
project_hash = "7ff47b1ee31c7f5f325d18abd52051b519f329dd"
